...
import           GHC.Types.Name.Reader        (mkVarUnqual, RdrName)
...
grhssSpan' (GRHSs {..}) =
  ...
  -- Combine them all into a single span.
   in combineSpans (binds_span : grhs_spans)
...
blankGRHSs grhss = do
  ...
  pure grhss { grhssGRHSs = [gr]         -- replace bodies
